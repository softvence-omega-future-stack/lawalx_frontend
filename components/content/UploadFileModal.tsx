...
export default function UploadFileModal({
    ...
    // ─── Returns a promise that resolves when simulation hits 100% ────────────
                ...
                const interval = setInterval(() => {
                    setFiles((prev) => {
                        ...
                        if (nextProgress >= 100) {
                            ...
                            return prev.map((f) =>
                                f.id === id
                                    ? { ...f, progress: 100, uploaded: fileSize }
                                    : f
                            );
                        }
    ...
    const handleDone = async () => {
        ...
        try {
            for (const entry of files) {
                // 1️⃣ Run progress bar to 100% first
                await runProgressSimulation(entry.id, entry.file.size);

                // 2️⃣ Progress is 100% — now call the API
...
//   const [isDragging, setIsDragging] = useState(false);
//   const fileInputRef = useRef<HTMLInputElement>(null);
//   // store per-file interval refs for progress simulation
//   const progressIntervalsRef = useRef<Record<string, ReturnType<typeof setInterval>>>({});
...
//   // Simulate upload progress for a file entry (stops at 90% — real 100% set after API)
//   const startProgressSimulation = useCallback((id: string, fileSize: number) => {
...
//       setFiles((prev) => {
//         const entry = prev.find((f) => f.id === id);
//         if (!entry || entry.progress >= 90) {
//           clearInterval(interval);
//           return prev;
//         }
//         // increment by random 5–15% chunks for natural feel
//         const increment = Math.floor(Math.random() * 10) + 5;
//         const nextProgress = Math.min(entry.progress + increment, 90);
//         const nextUploaded = Math.round((nextProgress / 100) * fileSize);
//         return prev.map((f) =>
//           f.id === id
//             ? { ...f, progress: nextProgress, uploaded: nextUploaded }
...
//   // Add files to state only — NO API call here
...
//   // ─── handleDone — single API call per file, XHR-style progress simulation ──
//   const handleDone = async () => {
//     if (files.length === 0) {
//       onClose();
//       return;
...
//         startProgressSimulation(entry.id, entry.file.size);
...
//           // Stop simulation, set to 100% done
//           stopProgressSimulation(entry.id);
//           setFiles((prev) =>
//             prev.map((f) =>
//               f.id === entry.id
//                 ? {
